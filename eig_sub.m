function xi = eig_sub(M, theta, phi, RY, preci, theta_d_max, theta_range, phi_d_max, phi_range, u)
Mx = sqrt(M);
Eta = zeros(4, 1);
%%%%%%%%%%%%%%%%%%
alpha_min1 = min(theta) - theta_range;
alpha_max1 = min(theta) + theta_range;
alpha_min2 = max(theta) - theta_range;
alpha_max2 = max(theta) + theta_range;
...
alpha_num2 = ceil(alpha_num2);
alpha_dnum = ceil(alpha_dnum);
%%%%%%%%%%%%%%%%%%%%%%
beta_min1 = min(phi) - phi_range;
beta_max1 = min(phi) + phi_range;
beta_min2 = max(phi) - phi_range;
beta_max2 = max(phi) + phi_range;
beta_num1 = (beta_max1 - beta_min1) / preci + 1;
beta_num2 = (beta_max2 - beta_min2) / preci + 1;
beta_dnum = phi_d_max / preci;%doa spread range 1`10 degrees
beta_num1 = ceil(beta_num1);
beta_num2 = ceil(beta_num2);
beta_dnum = ceil(beta_dnum);
%%%%%%%%%%%%%%%%%%%%%%%%%
Lambda1 = zeros(alpha_num1 * alpha_dnum * beta_num1 * beta_dnum, 1);%store of the correlations
Lambda2 = zeros(alpha_num2 * alpha_dnum * beta_num2 * beta_dnum, 1);%store of the correlations
Lambda_sub1 = zeros(alpha_num1 * alpha_dnum * beta_num1 * beta_dnum, 1);%store of the correlations
Lambda_sub2 = zeros(alpha_num1 * alpha_dnum * beta_num1 * beta_dnum, 1);%store of the correlations
[E, D] = eig(RY);
lambda = diag(D);
...
for ii = 1 : alpha_num2
    thetai = (alpha_min2 + (ii - 1) * preci) / 180 * pi;
    for jj = 1 : alpha_dnum
        theta_d = jj * preci / 180 * pi;
        for kk = 1 : beta_num2
            phii = (beta_min2 + (kk - 1) * preci) / 180 * pi;
            for ll = 1 : beta_dnum
                phi_d = ll * preci / 180 * pi;
                Psi = eye(M);
                for k = 2 : M
                ...
                Psi1 = E_noise' * Psi;
                Lambda2((((ii - 1) * alpha_dnum + jj - 1) * beta_num2 + kk - 1) * beta_dnum + ll, 1) = sum(diag(Psi1' * Psi1));
                Psi1 = RY \ Psi;
                Lambda_sub2((((ii - 1) * alpha_dnum + jj - 1) * beta_num2 + kk - 1) * beta_dnum + ll, 1) = sum(diag(Psi1' * Psi1));
            end
        end
    end
end
...
IX1 = (IX1 - ll) / beta_dnum;
kk = rem(IX1, beta_num2) + 1;
IX1 = (IX1 - kk + 1) / beta_num2;
jj = rem(IX1, alpha_dnum) + 1;
IX1 = (IX1 - jj + 1) / alpha_dnum;
ii = IX1 + 1;
Eta(1, 1) =  alpha_min2 + (ii - 1) * preci;
Eta(2, 1) = jj * preci;
Eta(3, 1) =  beta_min2 + (kk - 1) * preci;
Eta(4, 1) = ll * preci;
xi(:, 2) = Eta;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[~, IX] = sort(Lambda_sub1);
